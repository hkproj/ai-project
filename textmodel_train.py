...
from textmodel_datasets import ItaLipDataset, loadVocabulary, getSentenceFromInputIds
...
def getValidationInputFromBatch(batch: dict, startToken: int, previousInputIds: torch.Tensor) -> tuple:
    ...
    src_act_seq_len = batch['encoder_input_ids_len']
    ...
    tgt_act_seq_len = torch.tensor([tgt.size(1)]).type_as(batch['decoder_input_ids_len']).to(options['device'])
...
def getTrainingInputFromBatch(batch: dict) -> tuple:
    ...
    tgt_act_seq_len = batch['decoder_input_ids_len']
...
if __name__ == '__main__':
    ...
    # Create the model
    model = ItaLipModel(len(vocabulary), src_seq_len=options['max_sentence_len'], tgt_seq_len=options['max_sentence_len'], padding_idx=options['padding_idx'], nhead=options['nhead']).to(options['device'])
